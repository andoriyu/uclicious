...
impl UclErrorType {
    pub fn from_code(num: i32, desc: String) -> UclError {
        match num {
            _ if num == ucl_error_t::UCL_EOK       as i32 => UclError{code: UclErrorType::Ok, desc },
            _ if num == ucl_error_t::UCL_ESYNTAX   as i32 => UclError{code: UclErrorType::Syntax, desc },
            _ if num == ucl_error_t::UCL_EIO       as i32 => UclError{code: UclErrorType::Io, desc },
            _ if num == ucl_error_t::UCL_ESTATE    as i32 => UclError{code: UclErrorType::State, desc },
            _ if num == ucl_error_t::UCL_ENESTED   as i32 => UclError{code: UclErrorType::Nested, desc },
            _ if num == ucl_error_t::UCL_EMACRO    as i32 => UclError{code: UclErrorType::Macro, desc },
            _ if num == ucl_error_t::UCL_EINTERNAL as i32 => UclError{code: UclErrorType::Internal, desc },
            _ if num == ucl_error_t::UCL_ESSL      as i32 => UclError{code: UclErrorType::SSL, desc },
            _ => UclError{code: UclErrorType::Other, desc }
        }
    }
}
